...
import { 
  ArrowLeft, GitCommit, FileCode, Play, Plus, Trash2, Upload, Loader2, CheckCircle2 
} from "lucide-react";
...
export default function DeployPage() {
  ...
  const targetEnv = (searchParams.get("env") as AppEnvironment) || AppEnvironment.DEV;
    ...
    const fetchData = async () => {
      try {
        const [commitsRes, envsRes] = await Promise.all([
          ...
          api.get(`/api/components/${componentId}/envs`, { params: { env: targetEnv } })
        ]);
        ...
        if (Object.keys(existingEnvs).length > 0) {
          const formatted = Object.entries(existingEnvs).map(([key, value]) => ({ 
    ...
    <div className="max-w-4xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-3">
          Deploy to <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-lg text-lg border border-blue-200">{targetEnv}</span>
        </h1>
        <p className="text-slate-500 mt-2">Configure and launch a new build for your component.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className={`bg-white rounded-xl border transition-all duration-300 ${step === 1 ? 'border-blue-400 shadow-md ring-1 ring-blue-100' : 'border-slate-200 opacity-60'}`}>
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              {step > 1 && <Button size="sm" variant="outline" onClick={() => setStep(1)}>Change</Button>}
            </div>
              ...
              <div className="max-h-80 overflow-y-auto p-2">
                {commits.map((commit) => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-lg mb-2 border transition-all ${
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-xs text-slate-400">{new Date(commit.date).toLocaleDateString()}</span>
                    </div>
                    <p className="font-medium text-slate-800 text-sm line-clamp-1">{commit.message}</p>
          ...
          <div className={`bg-white rounded-xl border transition-all duration-300 ${step === 2 ? 'border-blue-400 shadow-md ring-1 ring-blue-100' : 'border-slate-200 opacity-60'}`}>
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="font-semibold text-slate-800 flex items-center gap-2">
              ...
              <div className="flex gap-2">
                 ...
                 {step > 2 && <Button size="sm" variant="outline" onClick={() => setStep(2)}>Change</Button>}
              </div>
            </div>
                ...
                {envVars.map((env, index) => (
                  <div key={index} className="flex gap-3">
                    <input
                      ...
                      className="flex-1 px-3 py-2 rounded border border-slate-200 text-sm font-mono focus:border-blue-500 outline-none"
                    />
                    <input
                      placeholder="VALUE"
                      value={env.value}
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm sticky top-24">
            <h3 className="font-bold text-slate-800 mb-4">Deployment Summary</h3>
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-800">{envVars.filter(v => v.key).length} keys</span>
              </div>
            </div>
